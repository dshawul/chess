...
static uint64_t calc_key(const Board *B);
static uint64_t calc_checkers(const Board *B, unsigned kcolor);
static uint64_t hidden_checkers(const Board *B, unsigned find_pins, unsigned color);
...
void set_fen(Board *B, const char *fen)
{
	...
	if (c == '-')	// no castling rights
		...
		if (c == 'K') { B->st->crights ^= OO; c = *fen++; }
		if (c == 'Q') { B->st->crights ^= OOO; c = *fen++; }
		if (c == 'k') { B->st->crights ^= OO << 2; c = *fen++; }
		if (c == 'q') { B->st->crights ^= OOO << 2; c = *fen++; }
	}
